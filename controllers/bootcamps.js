...
exports.getBootcamp = asyncHandler(async (req, res, next) => {
  const bootcamp = await await Bootcamp.findById(req.params.id).populate({
...
exports.createBootcamp = asyncHandler(async (req, res, next) => {
  ...
  if (publishedBootcamp && req.user.role !== 'admin') {
    return next(
      new ErrorResponse(`${req.user.id} has already published a bootcamp`, 400)
    );
  }
...
exports.bootcampPhotoUpload = asyncHandler(async (req, res, next) => {
  ...
  if (file.size > process.env.MAX_FILE_UPLOAD) {
    return next(
      new ErrorResponse(
        `File size ${file.size} too big. Upload image lte ${process.env.MAX_FILE_UPLOAD}`,
        400
      )
    );
  }
